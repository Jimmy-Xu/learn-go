...
# 3.append rbd to docker source
# 4.run docker with -cpuprofile
#############################################
...
function fn_patch_for_rbd(){
  echo "-------------------------------------"
  echo " > patch for rbd..."
  cp ${CUR_DIR}/src/* ${DOCKER_BASE}/ -R

  grep "librbd-dev librados-dev" ${DOCKERFILE} > /dev/null 2>&1
  if [ $? -ne 0 ];then
    sed -i '/COPY . \/go\/src\/github.com\/docker\/docker/i RUN apt-get install -y librbd-dev librados-dev' ${DOCKERFILE}
  else
    echo "Already patched for rbd"
  fi
  cd ${DOCKER_BASE} && git status && cd -
  if [ $? -ne 0 ];then
    quit "add rbd failed!"
  fi
}

function fn_patch_makefile(){
  echo "-------------------------------------"
  echo " > patch makefile: ${DOCKER_BASE}/Makefile"
  sed -i 's/\$(DOCKER_RUN_DOCKER) hack\/make.sh binary$/\$(DOCKER_RUN_DOCKER) hack\/make.sh dynbinary/' ${DOCKER_BASE}/Makefile
}
